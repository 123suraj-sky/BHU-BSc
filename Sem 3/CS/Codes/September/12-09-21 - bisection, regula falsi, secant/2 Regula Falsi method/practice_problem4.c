// Find a real root of equation f(x)=xe^x-1=0 using Regula Falsi method correct to two decimal places
...
{
    float y = (*x * exp(*x)) - 1; // given function
    if (y < 0)
        *a = *x;
    else if (y > 0)
        *b = *x;
...
int main()
{
    float a = 0, b = M_LN2; // f(a) < 0 and f(b) > 0
    float approx = 0;
    int count = 0;
